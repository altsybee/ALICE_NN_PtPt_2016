...
void toy_corrections_on_pt_in_wins_REAL_EFF_MAP() // TString inputFileName = "MergedOutput.root")
{
    // ########## Select and initiate windows:
    //    const int nEtaWins = 3;//2;

    TFile *fEff_HIJING = new TFile( "fileEffMap.root" );
    TH3D *hist3D_Eff = (TH3D *) fEff_HIJING->Get( "hist3D_effMap" );

    hist3D_Eff->Draw();

    TH1D *hist_check_eff = new TH1D( "hist_check_eff", "hist_check_eff", 20, 0.2, 2.0 );

//    int binEff = hist3D_Eff->GetXaxis()->FindBin(0.99);
//    cout << binEff << endl;
//    cout << hist3D_Eff->GetBinContent(binEff) << endl;
//    return;

    const int nRuns = 8;//12;//22;//18;
    ...
    for ( int run = 0; run < nRuns; run++ )
    {
        ...
        double ineff = 0.5*run;//0.25*run; //run*0.05;
...
//                        continue;
                ...
                int globalBinEff = hist3D_Eff->FindBin( 0.22, pt, 55 );
                //cout << binEff << endl;
                //cout << hist3D_Eff->GetBinContent(binEff) << endl;
                float effFromHist = hist3D_Eff->GetBinContent(globalBinEff);

                hist_check_eff->SetBinContent( hist_check_eff->FindBin(pt), effFromHist );
...
//                float effMod = 1.;
//                for ( int trial = 0; trial < run; trial++ )
//                {
//                    effMod *= 1-0.25*(1-effFromHist);
//                    if ( gRandom->Uniform() > effMod )
//                        isRejected = true;
//                }
//                if ( isRejected )
//                    continue;
...
//    fFitFunc->SetLineColor(kOrange-5+5);
    ...
    TCanvas *canv_eff_check = new TCanvas("canv_eff_check","canv_eff_check",450,250,700,600 );
    tuneCanvas(canv_eff_check);

    hist_check_eff->DrawCopy();
