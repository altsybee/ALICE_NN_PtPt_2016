...
void get_QA_plots_from_MergedOutputs_LHC10h_plus_minus() // TString inputFileName = "MergedOutput.root")
{
    //    TFile *files[10000];
    int nFiles = 0;

    TFile *files = list_files( nFiles, "/Users/macbook/alice/aliceAnalysis/results/task_2016_02_26_PbPb_Data_LHC10h_AOD160_8eW_phi1_pt02_20_FB_TREE_MFplus_all"
                               , "AnalysisResults" );
    ...
    for ( int i = 0; i < 3; i++ )
    {
        ...
        //        TH1D *hist = getFromFile( files[i] );
        //        hist->SetLineColor( kBlue );//colors[i] );
        //        tuneHist1D(hist);
        //hist->DrawNormalized( i == 0 ? "" : "same" );

        //        leg->AddEntry(hist, strLegend[i], "l");
    }
    //    leg->Draw();
