...
void draw_vertexZ_LHC10h_ALL_EVENTS()
{
    //    gROOT->ProcessLine(".L ../utils.C");

    TFile *f[9];

    f[0] = new TFile( "output_histos_graphs_vertexCut10.root" );
    f[1] = new TFile( "output_histos_graphs_vertexCut8.root" );
    f[2] = new TFile( "output_histos_graphs_vertexCut5.root" );
    f[3] = new TFile( "output_histos_graphs_vertexIn0_8.root" );
    f[4] = new TFile( "output_histos_graphs_vertexIn-8_0.root" );
    f[5] = new TFile( "output_histos_graphs_vertexAbove5.root" );
    f[6] = new TFile( "output_histos_graphs_vertexAbove5_below8.root" );


    const int nFiles = 7;

    TString strNumEventsInFiles[] = { "|vertexZ| < 10", "|vertexZ| < 8"
          , "|vertexZ|<5", "vertexZ#in(0,8)", "vertexZ#in(-8,0)", "|vertexZ|>5"
          , "|vertexZ|>5 && <8" };
...
//                graphs[fId][cW][etaW] = (TGraphErrors*)f[fId]->Get( Form( "gr_BS_PtPt_cW%d_etaW%d_phiW0", cW, etaW ) );
    ...
    int colors[] = { kBlue, kMagenta, kRed, kCyan, kGray, kOrange, kGreen+1 };
    int markers[] = { 20, 24, 21, 25, 29, 30, 28 };
